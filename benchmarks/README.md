...
```bash
uv run python benchmarks/run_all.py --full          # ClickBench 用完整 sorted 数据集
uv run python benchmarks/run_all.py --skip-build    # 扩展已最新，跳过重新构建
uv run python benchmarks/run_all.py --only core     # 只跑核心微基准
uv run python benchmarks/run_all.py --only vs        # 只跑 ClickBench 对比
uv run python benchmarks/run_all.py --prepare        # 运行前显式准备数据（可能下载 ~14GB）
```
