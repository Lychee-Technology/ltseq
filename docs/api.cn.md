...
| `TypeError: LTSeq expressions cannot be used in a boolean context` | 对行表达式或组谓词使用了 `and`/`or`/`not`/`in`/三元/链式比较，如 `(r.a > 2) and (r.b < 1.5)` 或 `(g.count() > 2) and (g.sum("x") > 0)` | 用 `&` `\|` `~` 组合条件，如 `(r.a > 2) & (r.b < 1.5)`；`in` 改用 `.is_in([...])` |
