...
1. **Capture.** A lambda is executed **once** against a `SchemaProxy` (`expr/proxy.py`), not against real rows. Attribute access and operator overloads build an expression tree (`ColumnExpr`, `BinOpExpr`, `CallExpr`, …).
...
**The DSL is intentionally constrained**: it supports Pythonic *column* expressions well and deliberately does not execute arbitrary Python row by row, a tradeoff that keeps expressions serializable and Rust-executable. What Python cannot overload gets a limited AST transformation (`is None` / `is not None`, in `expr/transforms.py`). The escape hatch for genuinely arbitrary per-row logic is `fold()`, an explicitly-flagged slow path ([ADR 0005](0005-no-materialization-rule.md)).
...
- `api.md` §11 documents several expression semantics by their SQL equivalents (e.g. `if_else` → `CASE WHEN`, `count_if` → `SUM(CASE WHEN …)`). Those are specification devices; execution is native.
