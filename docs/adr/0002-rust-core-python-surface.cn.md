...
- **Python 层**（`py-ltseq/ltseq/`）：公开 API 表面。负责表达式捕获（见 [ADR 0007](0007-lambda-dsl-schemaproxy.cn.md)）、schema 与排序元数据的簿记、以及决定返回哪种包装对象（见 [ADR 0010](0010-four-table-object-types.cn.md)）。这一层刻意保持"薄"，不得自己做重型数据处理。
...
- 任何计划形状的变更都要跨越边界，并要求两侧元数据重新同步——尤其是双 schema 模型（[ADR 0009](0009-dual-schema-model.cn.md)）与排序元数据（[ADR 0008](0008-explicit-sort-metadata.cn.md)）。
