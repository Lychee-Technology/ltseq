...
- **The stub flattens the mixins**: runtime `LTSeq` is mixin-composed ([ADR 0012](0012-rust-thin-shell-python-mixins.md)), but `__init__.pyi` declares one flat `LTSeq` class with all methods. That divergence between declared and runtime structure is deliberate, chosen for IDE ergonomics. Lambdas are typed `Callable[[SchemaProxy], Expr]`; aliases (`with_columns`, `group_consecutive`) are included; factory functions get full signatures.
