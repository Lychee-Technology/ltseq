...
- Status: Accepted
- Date: 2026-04-05 (design spec date)
...
- `SchemaProxy.__getattr__(name) -> ColumnExpr`: statically promise `ColumnExpr` for any attribute (runtime still validates column existence).
- **Explicit-plus-fallback on `ColumnExpr`/`CallExpr`**: declare the most common methods explicitly for discoverability (window ops `shift`/`rolling`/`diff`; aggregations `sum`/`mean`/`min`/`max`/`count`/`std`/`var`/`first`/`last`/`median`; rolling-chain targets), then add `def __getattr__(self, name: str) -> Callable[..., CallExpr]` as a fallback. This is the central trade-off: the explicit list buys IDE discoverability, the fallback preserves the dynamic runtime — at the cost that anything *not* listed type-checks as valid.
...
- Private methods and internal helpers are not stubbed.
...
- `SchemaProxy.__getattr__` was implemented as `-> ColumnExpr | NestedSchemaProxy` (spec said `ColumnExpr`).
- Stubs use `TYPE_CHECKING` guards to break circular imports (stated only in the implementation plan).
- Stubs have no unit tests — the "test" is that importing `ltseq` succeeds and the suite stays green; no mypy/pyright CI gate was added.
