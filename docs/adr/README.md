...
| 0016 | [Release the GIL around execution](0016-gil-release-execution-boundary.md) · [中文](0016-gil-release-execution-boundary.cn.md) | Parse under the GIL, execute through `gil::detached` returning `LtseqError`, convert after; cursor locks inside the detached section. (#142) / 持 GIL 解析，经 `gil::detached` 执行并返回 `LtseqError`，之后再转换；cursor 在 detach 段内取锁。（#142） |
| 0017 | [Arrow C Data Interface boundary](0017-arrow-c-data-interface-boundary.md) · [中文](0017-arrow-c-data-interface-boundary.cn.md) | Shared Arrow buffers over the C Data Interface replace the IPC tunnel; `from_arrow` takes any `__arrow_c_stream__` object, `LTSeq` exports one; lazy stream semantics fixed. (#143) / 以 C Data Interface 共享 Arrow buffer 取代 IPC 隧道；`from_arrow` 接受任何 `__arrow_c_stream__` 对象，`LTSeq` 也导出该协议；惰性流语义已定。（#143） |
