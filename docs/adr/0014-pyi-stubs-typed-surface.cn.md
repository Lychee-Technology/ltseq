...
- **Stub 把 mixin 摊平**：运行时 `LTSeq` 由 mixin 组合（[ADR 0012](0012-rust-thin-shell-python-mixins.cn.md)），但 `__init__.pyi` 声明的是一个包含全部方法的扁平 `LTSeq` 类。声明结构与运行时结构的这处分叉是有意的，为 IDE 人体工学服务。lambda 参数类型为 `Callable[[SchemaProxy], Expr]`；别名（`with_columns`、`group_consecutive`）包含在内；工厂函数给出完整签名。
