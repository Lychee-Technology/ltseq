...
显式跟踪排序状态——声明顺序由 Rust 内核持有（`sort_specs`），Python 的 `_sort_keys` 经 FFI 读取（[ADR 0009](0009-metadata-single-source-of-truth.cn.md)）——并且**从不隐式推断顺序**。顺序未知时，LTSeq 宁可报错（`SortRequiredError`）也不静默产出误导性结果。如果结果依赖顺序，顺序就应该出现在代码里。
