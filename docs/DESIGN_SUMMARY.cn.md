...
在 Rust 侧，这些会在 `src/types.rs` 中被还原为 `PyExpr`。字面量载荷带类型：`LiteralExpr` 在捕获时校验 Python 值（不支持的类型抛出 `TypeError`），Rust 按 `dtype` 提取对应的原生值，存为 `LiteralValue`，不会先转成字符串再解析回来。
...
### 4.1 前缀别名 LinkedTable

`link()` 返回的是 `LinkedTable`，即一个延迟的前缀别名等值连接，而不是立刻拍平后的 join 结果表。

每个变换都会构建惰性 join 计划并返回普通 `LTSeq`，其行数遵循 join 语义（含不匹配行与一对多扇出）；计划在终端操作前保持惰性。这是惰性 join，不是指针/take 结构。
