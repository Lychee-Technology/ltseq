...
A contributor-oriented map of the codebase, written to answer one question: if you need to change a feature, where do you start reading?
...
This file also contains the limited AST transformation used for `is None` and `is not None` inside lambdas. Python cannot overload `is`, so `_transform_lambda_for_none_checks` locates the lambda in its module's AST (each lambda on the line is compiled in the same context as the original: the same free variables, the defining module's `from __future__` features, and the same enclosing class so private names mangle identically; the result is compared with the function's code object ignoring only the nesting flag, so several lambdas on one line, f-strings, multi-line lambdas, and module-scope lambdas are unambiguous), rewrites each `x is None` / `x is not None` into a call that yields `x.is_null()` / `x.is_not_null()` for expressions and a plain identity check otherwise, and rebuilds the function on the original `__globals__` and closure cells (a `functools.partial` of a lambda is rebuilt around the rewritten lambda with the same bound arguments). The rewrite is applied by `_invoke_row_lambda`, the single entry point through which every public API that accepts `lambda r: ...` (`filter`, `derive`, `select`, `delete`, `update`, `sort`, ...) runs the lambda against a `SchemaProxy`; a new API must call it rather than build its own proxy, and must capture the user's lambda itself rather than a wrapper lambda around it. A function whose null identity check was not rewritten (source unavailable in a REPL or `exec`/`eval` string, a plain `def`, or source that no longer matches the loaded lambda) is never run: `_invoke_row_lambda` reads its bytecode and raises a `TypeError` that points at `.is_null()` / `.is_not_null()`, because the unrewritten check is a Python bool that `and`, `or`, or a conditional expression would otherwise silently fold out of the captured expression. A check is an `is` / `is not` whose source span (from the code object's position table, so no source file is needed) encloses a load of the `None` constant, or the jump the compiler emits for `x is None` used as a branch condition; an identity test between other values (`flag is marker`) next to an unrelated `None` literal is not one. The check covers the function's own code, the functions nested in it, and the functions a wrapper forwards to (`functools.partial`, `staticmethod`, `functools.lru_cache`, and any wrapper that sets `__wrapped__`, since the ones written in C have no code of their own), but not helpers it calls, and without source it cannot tell a column from a Python value, so it rejects an `is None` test on a Python value in such a function too.
...
The tests are a good entrypoint into the codebase, because they are organized by product capability.
...
These are worth more than memorizing every file.
