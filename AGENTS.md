...
**Lazy evaluation**: LinkedTable and NestedTable are deferred wrappers; constructing one does no join or grouping work. LinkedTable transforms stay on the lazy DataFusion join plan, and NestedTable's `flatten()`, `derive()`, `filter()`, `first()`, and `last()` route through `group_id_impl`, which deliberately uses the native window path rather than linear scan. Laziness can still end on consumption. `first().count()` tries the `group_ordered_count` fast path, which accepts only shift-based boundary predicates (`can_linear_scan` requires a `shift` node); on an eligible predicate outside pre-sorted Parquet it runs `general_linear_scan_group_id`, which collects. An ineligible predicate raises, and Python falls back to materializing via `flatten().first_row()`. ADR 0005 is the inventory of these boundaries.
